...
# 🚀 Smart Auto-Start with Dynamic ngrok URL Detection
# This script automatically handles changing ngrok URLs

set -e

echo "🎵 Mood Melodies App - Smart Auto-Start with Dynamic ngrok"
echo "=========================================================="
...
get_ngrok_url() {
    cd face-api-service
    if node get-ngrok-url.js 2>/dev/null; then
        return 0
    else
        return 1
    fi
...
if check_service "ngrok tunnel" "curl -s http://localhost:4040/api/tunnels"; then
    ...
    if [ $? -eq 0 ] && [ ! -z "$CURRENT_NGROK_URL" ]; then
        echo -e "${GREEN}🌐 Current ngrok URL: ${CURRENT_NGROK_URL}${NC}"
    else
        echo -e "${YELLOW}⚠️ ngrok is running but no tunnel URL found${NC}"
        NGROK_RUNNING=false
    fi
else
    NGROK_RUNNING=false
...
if [ "$NGROK_RUNNING" = false ]; then
    ...
    echo -e "${BLUE}Getting new ngrok tunnel URL...${NC}"
    sleep 3  # Give ngrok a moment to establish tunnel
    
    NEW_NGROK_URL=$(get_ngrok_url)
    if [ $? -eq 0 ] && [ ! -z "$NEW_NGROK_URL" ]; then
        echo -e "${GREEN}🌐 New ngrok URL: ${NEW_NGROK_URL}${NC}"
        
        # Update status file
        cd face-api-service
        cat > NGROK-STATUS.md << EOF
🚀 **NGROK TUNNEL ACTIVE - REAL FACE ANALYSIS ENABLED!**
...
- Ngrok tunnel: ✅ **${NEW_NGROK_URL}**

🌐 **Current Status:**

- **Expo Tunnel**: ✅ Running (your phone app)
- **Face API Tunnel**: ✅ **${NEW_NGROK_URL}**
...
- Face API: \`${NEW_NGROK_URL}\`

**Auto-updated on:** $(date)
EOF
        cd ..
        
        echo -e "${GREEN}✅ Updated NGROK-STATUS.md with new URL${NC}"
    else
...
echo ""
echo -e "${YELLOW}💡 The app automatically detects the current ngrok URL${NC}"
